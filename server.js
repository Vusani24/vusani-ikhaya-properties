...
async function api(req, res, url) {
  ...
  if (url.pathname.startsWith("/api/admin/")) {
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/action") {
      ...
      if (body.action === "move") moveItem(db, body.section, body.from, body.to, body.id);
      else if (body.action === "delete") deleteItem(db, body.section, body.from, body.id);
        ...
        const room = db.rooms.pending.find((entry) => entry.id === body.id);
        if (room) {
          room.createdAt = room.createdAt || new Date().toISOString();
          delete room.takenDate;
          db.rooms.pending = db.rooms.pending.filter((entry) => entry.id !== body.id);
          db.rooms.approved.unshift({ ...room, status: "approved" });
        }
      } else if (body.action === "decline") {
        deleteItem(db, "rooms", "pending", body.id);
      } else if (body.action === "remove") {
        deleteItem(db, "rooms", "approved", body.id);
      } else if (body.action === "taken") {
        const room = db.rooms.approved.find((entry) => entry.id === body.id);
        if (room) {
          room.takenDate = new Date().toISOString();
          db.rooms.approved = db.rooms.approved.filter((entry) => entry.id !== body.id);
          db.rooms.taken.unshift({ ...room, status: "taken" });
        }
      ...
      } else if (body.action === "remove-taken") {
        deleteItem(db, "rooms", "taken", body.id);
      } else if (body.action === "approve-review") {
        const review = db.reviews.pending.find((entry) => entry.id === body.id);
        if (review) {
          db.reviews.pending = db.reviews.pending.filter((entry) => entry.id !== body.id);
          db.reviews.approved.unshift({ ...review, status: "approved" });
        }
      } else if (body.action === "decline-review") {
        deleteItem(db, "reviews", "pending", body.id);
      } else if (body.action === "approve-report") {
        const report = db.reports.pending.find((entry) => entry.id === body.id);
        if (report) {
          db.reports.pending = db.reports.pending.filter((entry) => entry.id !== body.id);
          db.rooms.approved = db.rooms.approved.filter((room) => room.title !== report.room && room.address !== report.room);
          db.rooms.pending = db.rooms.pending.filter((room) => room.title !== report.room && room.address !== report.room);
          db.rooms.taken = db.rooms.taken.filter((room) => room.title !== report.room && room.address !== report.room);
        }
      } else if (body.action === "decline-report") {
        deleteItem(db, "reports", "pending", body.id);
      } else if (body.action === "edit-room") {
        const { updates = {} } = body;
        const room = [...db.rooms.pending, ...db.rooms.approved, ...db.rooms.taken].find((entry) => entry.id === body.id);
        if (room) Object.assign(room, updates);
      } else if (body.action === "update-availability") {
        const room = [...db.rooms.pending, ...db.rooms.approved, ...db.rooms.taken].find((entry) => entry.id === body.id);
        if (room) room.availableDate = body.availableDate || room.availableDate;
      }
      writeDB(db);
