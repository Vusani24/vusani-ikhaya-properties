...
const DATABASE_URL = process.env.DATABASE_URL || process.env["DATABASE-URL"] || "";
...
function getPostgresPool() {
  ...
  if (postgresAttempted) return postgresPool;
  postgresAttempted = true;
  try {
    const { Pool } = require("pg");
    const cleanDatabaseURL = new URL(DATABASE_URL);
    cleanDatabaseURL.searchParams.delete("sslmode");
    cleanDatabaseURL.searchParams.delete("channel_binding");
    postgresPool = new Pool({
      connectionString: cleanDatabaseURL.toString(),
    ...
    console.warn("DATABASE_URL is configured but pg is not installed. Falling back to Supabase or local db.json.");
  }
  return postgresPool;
}
...
async function readDB() {
  ...
  if (pool) {
    try {
      ...
      const result = await pool.query("select data from app_state where key = $1", [SUPABASE_STATE_KEY]);
      ...
      console.error("Postgres read failed. Falling back to local db.json:", error.message);
      return readLocalDB();
    }
  }
  ...
  if (pool) {
    try {
      ...
      await pool.query(
        ...
        [SUPABASE_STATE_KEY, JSON.stringify(nextDB)]
      );
      return;
    } catch (error) {
      console.error("Postgres write failed. Saving to local db.json:", error.message);
      writeLocalDB(nextDB);
      return;
    }
  }
...
async function databaseStatus() {
  ...
  if (pool) {
    try {
      await ensurePostgresSchema(pool);
      await pool.query("update app_state set updated_at = updated_at where key = $1", [SUPABASE_STATE_KEY]);
      const result = await pool.query("select key, updated_at from app_state where key = $1", [SUPABASE_STATE_KEY]);
      return {
        mode: "postgres-database-url",
        databaseUrlConfigured: true,
        ok: true,
        canWrite: true,
        hasMainState: Boolean(result.rows[0]),
...
function requireAdmin(req, res) {
  ...
  if (!token || (token !== adminToken() && !sessions.has(token))) {
...
function adminToken() {
  return crypto
    .createHash("sha256")
    .update(`alexandra-admin:${ADMIN_PASSWORD}`)
    .digest("hex");
}
...
function deleteEverywhere(db, id) {
  ["pending", "approved", "taken", "declined", "removed"].forEach((status) => {
    db.rooms[status] = db.rooms[status].filter((entry) => entry.id !== id);
  });
}
...
async function api(req, res, url) {
  if (req.method === "GET" && url.pathname === "/api/public") {
    try {
      ...
      send(res, 200, { rooms: [], reviews: [], reports: [], transports: [], warning: "Temporary database issue" });
    }
    return;
  }
  ...
  if (req.method === "POST" && url.pathname === "/api/visit") {
    try {
      ...
      send(res, 201, { ok: true, warning: "Visit tracking skipped" });
    }
    return;
  }
  ...
  if (req.method === "POST" && url.pathname === "/api/rooms") {
    ...
    send(res, 201, {
      ok: true,
      pendingCount: db.rooms.pending.length,
      message: `Room submitted to admin. Pending rooms waiting for approval: ${db.rooms.pending.length}.`
    });
    return;
  }
  ...
  if (url.pathname.startsWith("/api/admin/")) {
    ...
    if (req.method === "GET" && url.pathname === "/api/admin/pending-preview") {
      return send(res, 200, {
        pending: db.rooms.pending.map((room) => ({
          id: room.id,
          title: room.title,
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/action") {
      ...
      if (body.action === "create-test-pending") {
        db.rooms.pending.unshift({
          id: "admin-test-" + Date.now(),
          title: "Admin Test Pending Room",
          address: "Test address, Alexandra",
          location: "Alexandra",
          type: "Single room",
          amount: "R1000",
          deposit: "No deposit stated",
          childFriendly: "Yes",
          parking: "No",
          bath: "Sharing bathroom",
          images: [],
          video: "",
          posterName: "Admin test landlord",
          posterContact: "0613642449",
          notes: "This is a test post created from admin to confirm pending posts are working.",
          status: "pending",
          createdAt: new Date().toISOString()
        });
      }
      ...
      if (body.action === "delete") {
        if (body.section === "rooms") {
          deleteEverywhere(db, body.id);
        } else {
...
function serveFile(req, res, url) {
  ...
  const cache = [".html", ".js"].includes(ext) ? "no-store" : "public, max-age=3600";
